...
# from scripts.AMP.utils.utils import Normalizer
...
from utils.amp_states_collection import collect_amp_states_from_obs
from utils.load_motion_dataset import load_motion_dataset
import numpy as np
import os
...
class AMP(Agent):
    def __init__(
        ...
        self.collect_reference_motions = self._collect_reference_motions
        ...
        if self._amp_state_preprocessor:
            ...
            self._amp_state_preprocessor = Normalizer(amp_observation_space//2)
        
        # load motion dataset
        self.motion_tensor = load_motion_dataset(amp_observation_space).to(self.device) 
    ...
    def record_transition(
        ...
        if self.memory is not None:
            ...
            amp_states = collect_amp_states_from_obs(states, next_states)
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
            amp_logits, _, _ = self.discriminator.act(
                {"states": torch.cat([self._amp_state_preprocessor.normalize_torch(amp_states[:,:,:self.amp_observation_space//2],device=self.device),self._amp_state_preprocessor.normalize_torch(amp_states[:,:,self.amp_observation_space//2:],device=self.device)],dim=-1).detach()}, role="discriminator"
            )
        ...
        for epoch in range(self._learning_epochs):
            ...
            for batch_index, (
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    # --- CORRECTED POLICY EVALUATION ---
                    # Evaluate policy on original states to get actions and log_prob for BOTH losses
                    ...
                    if self._discriminator_batch_size:
                        sampled_amp_states = self._amp_state_preprocessor(
                            sampled_amp_states[0 : self._discriminator_batch_size]
                        )
                        sampled_amp_replay_states = self._amp_state_preprocessor(
                            sampled_replay_batches[batch_index][0][0 : self._discriminator_batch_size]
                        )
                        sampled_amp_motion_states = self._amp_state_preprocessor(
                            sampled_motion_batches[batch_index][0][0 : self._discriminator_batch_size]
                        )
                    else:
                        sampled_amp_states = torch.cat(
                                [
                                    self._amp_state_preprocessor.normalize_torch(
                                        sampled_amp_states[:,:self.amp_observation_space//2],
                        ...
                        sampled_amp_replay_states = torch.cat(
                            [
                                self._amp_state_preprocessor.normalize_torch(
                                    sampled_replay_batches[batch_index][0][:,:][:,:self.amp_observation_space//2],
                                    device=self.device
                                ).detach(),
                                self._amp_state_preprocessor.normalize_torch(
                                    sampled_replay_batches[batch_index][0][:,:][:,self.amp_observation_space//2:],
                        ...
                        sampled_amp_motion_states = torch.cat(
                            [
                                self._amp_state_preprocessor.normalize_torch(
                                    sampled_motion_batches[batch_index][0][:,:self.amp_observation_space//2],
                                    device=self.device
                                ).detach(),
                                self._amp_state_preprocessor.normalize_torch(
                                    sampled_motion_batches[batch_index][0][:,self.amp_observation_space//2:],
                ...
                policy_state = sampled_amp_states[:,:self.amp_observation_space//2]
                expert_state = sampled_motion_batches[batch_index][0][:,:self.amp_observation_space//2]


                if self._amp_state_preprocessor is not None:
                    self._amp_state_preprocessor.update(policy_state.cpu().numpy())
                    self._amp_state_preprocessor.update(expert_state.cpu().numpy())
        ...
        self.reply_buffer.add_samples(states=amp_states.view(-1, amp_states.shape[-1])[:self.memory.num_envs,:])
    ...
    def _collect_reference_motions(self, batch_size: int) -> torch.Tensor:
        """Sample reference motions from dataset"""
        # Generate random indices
        indices = torch.randint(0, len(self.motion_tensor), (self.memory.num_envs,), device=self.device)
      
        # Return sampled motion pairs
        return self.motion_tensor[indices]
